...
public class PasswordEvaluator {
	 ...
	 * @version 0.01    2025-01-23  !!!Updated Password Evaluator FSM based on new requirements,
	 *                              removed some special characters from special characters
	 *                              list!!!
	 */
	 ...
	 * @return					Two lines, the entire input line followed by a 
	 * 							line with an up arrow
	 */
	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			displayInputState();
			// FSM State 0 Transitions: 
			// The cascading if statement sequentially tries the current character against all of the
			// valid transitions
			
			// Transition [1] Check for A-Z
			if (currentChar >= 'A' && currentChar <= 'Z') {
				System.out.println("Upper case letter found");
				foundUpperCase = true;
				
			// Transition [2] Check for a-z
			} else if (currentChar >= 'a' && currentChar <= 'z') {     
				System.out.println("Lower case letter found");
				foundLowerCase = true;
				
			// Transition [3] Check for 0-9
			} else if (currentChar >= '0' && currentChar <= '9') {                
				System.out.println("Digit found");
				foundNumericDigit = true;
				
			// Transition [4] Check for special characters
			} else if ("~`!@#$%^&*()_-+{}[]|:,.?/".indexOf(currentChar) >= 0) {   
				System.out.println("Special character found");
				foundSpecialChar = true;
				
			// Transition [5] Other characters found
			} else {                                                              
				passwordIndexofError = currentCharNdx;
				return "An invalid character has been found!";
			}
			
			// [6] Input is long enough
			if (currentCharNdx >= 7) {                                            
